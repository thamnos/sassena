/** \file
This file implements a monitoring service, where clients push updates towards to
head node, which then presents the user with up-to-date information about the
progress.
...
MonitorService::MonitorService(boost::asio::io_service& io_service, double from,
                               double to)
...
void MonitorService::listener() {
  ...
  while (true) {
    ...
    if (tag == MONITOR_UPDATE) {
      ...
      double sum = 0;
      for (std::map<size_t, double>::iterator pi = progresses.begin();
           pi != progresses.end(); pi++) {
        sum += pi->second;
      }
...
void MonitorService::print() {
  ...
  Info::Inst()->write(
      string("Progress: ") + ifraction_done_strstr.str() +
      string("%, ETOTAL: ") + boost::lexical_cast<string>(etotal) +
      string(" s, ETA: ") + boost::lexical_cast<string>(eta) + string(" s"));
}

MonitorClient::MonitorClient(boost::asio::ip::tcp::endpoint server)
    : m_endpoint(server) {
  // create a queue of update thresholds
...
void MonitorClient::update(size_t rank, double progress) {
  ...
  if (update_thresholds.size() != 0) {
    ...
    if ((boost::posix_time::second_clock::universal_time() - lastupdate_) <=
        (boost::posix_time::seconds(
            Params::Inst()->limits.services.monitor.delay))) {
      return;
    }
  }
  ...
  if (update_thresholds.size() != oldsize) {
    ...
    try {
      ...
      Warn::Inst()->write(
          "Increase limits.services.monitor.delay and/or "
          "limits.services.monitor.sampling");
      Warn::Inst()->write(string("Current sampling factor: ") +
                          boost::lexical_cast<string>(samplingfactor_));
    }
  }
}
