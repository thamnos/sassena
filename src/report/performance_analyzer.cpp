/** \file
This file contains an performance analysis class, which takes several timers and
prepares a formatted output.
...
PerformanceAnalyzer::PerformanceAnalyzer(
    boost::mpi::communicator thisworld,
    std::map<boost::thread::id, Timer>& timermap) {
  ...
  //	vector<double> all_sum, all_count, all_mean, all_variance, all_min,
  //all_max;
  ...
  for (std::map<boost::thread::id, Timer>::iterator i = timermap.begin();
       i != timermap.end(); ++i) {
  ...
  for (set<string>::iterator i = my_keys_set.begin(); i != my_keys_set.end();
       ++i) {
    my_keys.push_back(*i);
  }
  ...
  if (thisworld.rank() == 0) {
    for (size_t i = 0; i < all_keys.size(); ++i) {
      for (vector<string>::iterator j = all_keys[i].begin();
           j != all_keys[i].end(); ++j) {
  ...
  for (set<string>::iterator i = common_keys_set.begin();
       i != common_keys_set.end(); ++i) {
    common_keys.push_back(*i);
  }
  mpi::wrapper::broadcast_class<vector<string> >(thisworld, common_keys, 0);

  for (vector<string>::iterator ti = common_keys.begin();
       ti != common_keys.end(); ++ti) {
    ...
    for (std::map<boost::thread::id, Timer>::iterator thistimer =
             timermap.begin();
         thistimer != timermap.end(); ++thistimer) {
...
void PerformanceAnalyzer::report() {
  ...
  string mthead = (boost::format("%|17| |%|9| |%|12| |%|9| |%|9|") % "measure" %
                   "total" % "count" % "mean" % "stddev")
                      .str();
  string mmhead =
      (boost::format("%|17| |%|9| |%|9|") % "measure" % "min" % "max").str();
  ...
  for (map<string, PerformanceMeasure>::iterator mi = m_measures.begin();
       mi != m_measures.end(); mi++) {
...
void PerformanceAnalyzer::report_relative(double totaltime) {
  ...
  Info::Inst()->write(
      "                                                                 ");
  Info::Inst()->write(
      "         Performance Analysis (relative to total time * nodes)   ");
  Info::Inst()->write(
      "-----------------------------------------------------------------");
  Info::Inst()->write(
      " mean and total runtimes:                                        ");
  ...
  for (map<string, PerformanceMeasure>::iterator mi = m_measures.begin();
    ...
    form % group(setprecision(3), scientific,
                 sqrt(mi->second.variance / totaltime));
    Info::Inst()->write(form.str());
  }
  Info::Inst()->write(
      "-----------------------------------------------------------------");
...
//		string mthead = (boost::format("%|17| |%|9| |%|12| |%|9| |%|9|") %
//"measure" % "total" % "count" % "mean" % "stddev").str();
//		string mmhead = (boost::format("%|17| |%|9| |%|9|") % "measure" %
//"min" % "max").str();
//
//		Info::Inst()->write(" ");
