/** \file
This file contains a class which provides a management interface for
atomselections.
...
IAtomselection* Atomselections::operator[](std::string name) {
  ...
  if (it == _selections.end()) {
    Err::Inst()->write(
        string("Selection with that name has not been defined: ") + name);
    Err::Inst()->write(string(
        "Add a selection with the above name to your configuration file"));
    throw;
  }
  return it->second;
}
...
void Atomselections::set(std::string name, IAtomselection* selection) {
  if (_selections.find(name) != _selections.end()) {
    Err::Inst()->write(string("Selection with that name already exists: ") +
                       name);
...
void Atomselections::rename(std::string name, std::string newname) {
  ...
  if (it == _selections.end()) {
    Err::Inst()->write(
        string(
            "Cannot rename selection with that name which not been defined: ") +
        name);
    Err::Inst()->write(
        string("You shouldn't see this message. File a bug report."));
    throw;
  }
  if (_selections.find(newname) != _selections.end()) {
    Err::Inst()->write(string("Cannot rename selection to a name which has "
...
Atomselections::~Atomselections() {
  for (Atomselections::iterator i = _selections.begin(); i != _selections.end();
       ++i) {
    delete (i->second);
  }
}
