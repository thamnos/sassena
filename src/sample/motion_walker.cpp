/** \file
This file contains a set of artifical motion generators. They are used to
superimpose artifical with the specific dynamics provided by the trajectory
files.
...
RotationalBrownianMotionWalker::RotationalBrownianMotionWalker(
    double displace, unsigned long seed, long sampling)
    : m_init(true) {
  m_seed = seed;
  m_sampling = sampling;
  m_displace = displace;
...
boost::numeric::ublas::matrix<double> RotationalBrownianMotionWalker::transform(
    size_t timepos) {
...
void RotationalBrownianMotionWalker::generate(size_t timepos) {
  ...
  if (timepos < oldtimepos)
    return;  // don't generate anything if we already have it
  ...
  for (size_t ti = oldtimepos; ti <= timepos; ++ti) {
    ...
    transformations[ti] =
        boost::numeric::ublas::prod(oldtransform, newtransform);
...
BrownianMotionWalker::BrownianMotionWalker(double displace, unsigned long seed,
                                           long sampling,
...
boost::numeric::ublas::matrix<double> BrownianMotionWalker::transform(
    size_t timepos) {
  using namespace boost::numeric::ublas;
  matrix<double> T(4, 4);
  T = identity_matrix<double>(4);
...
void BrownianMotionWalker::generate(size_t timepos) {
  ...
  for (size_t ti = oldtimepos; ti <= timepos; ++ti) {
    ...
    translations[ti] =
        oldtranslation +
        displacement *
            CartesianCoor3D(sphereran[0], sphereran[1], sphereran[2]);
...
LocalBrownianMotionWalker::LocalBrownianMotionWalker(double radius,
                                                     double displace,
                                                     unsigned long seed,
                                                     long sampling,
                                                     CartesianCoor3D direction)
  ...
  if (m_displace > m_radius) {
    Err::Inst()->write(
        "radius size for local brownian motion smaller than displacement!");
    throw;
  }
}
...
boost::numeric::ublas::matrix<double> LocalBrownianMotionWalker::transform(
    size_t timepos) {
  using namespace boost::numeric::ublas;
  matrix<double> T(4, 4);
  T = identity_matrix<double>(4);
...
void LocalBrownianMotionWalker::generate(size_t timepos) {
  ...
  for (size_t ti = oldtimepos; ti <= timepos; ++ti) {
    ...
    while (true) {
      ...
      double displacement = m_displace * normran;
      newtrans = oldtranslation +
                 displacement *
                     CartesianCoor3D(sphereran[0], sphereran[1], sphereran[2]);
      if (newtrans.length() > m_radius)
        continue;
      else
        break;
    }
...
RandomMotionWalker::RandomMotionWalker(double displace, unsigned long seed,
                                       long sampling, CartesianCoor3D direction)
    : m_init(true) {
  m_seed = seed;
  m_sampling = sampling;
  m_direction = direction;
...
boost::numeric::ublas::matrix<double> RandomMotionWalker::transform(
    size_t timepos) {
  using namespace boost::numeric::ublas;
  matrix<double> T(4, 4);
  T = identity_matrix<double>(4);
...
void RandomMotionWalker::generate(size_t timepos) {
  ...
  for (size_t ti = oldtimepos; ti <= timepos; ++ti) {
    ...
    translations[ti] =
        oldtranslation +
        m_displace * CartesianCoor3D(sphereran[0], sphereran[1], sphereran[2]);
    oldtranslation = translations[ti];
  }
}
...
OscillationMotionWalker::OscillationMotionWalker(double displace,
                                                 double frequency,
                                                 long sampling,
                                                 CartesianCoor3D direction) {
...
boost::numeric::ublas::matrix<double> OscillationMotionWalker::transform(
    size_t timepos) {
  using namespace boost::numeric::ublas;
  matrix<double> T(4, 4);
  T = identity_matrix<double>(4);
...
LinearMotionWalker::LinearMotionWalker(double displace, long sampling,
                                       CartesianCoor3D direction) {
...
boost::numeric::ublas::matrix<double> LinearMotionWalker::transform(
    size_t timepos) {
  using namespace boost::numeric::ublas;
  matrix<double> T(4, 4);
  T = identity_matrix<double>(4);
...
FixedMotionWalker::FixedMotionWalker(double displace,
                                     CartesianCoor3D direction) {
  m_translate = displace * direction / direction.length();
}
...
boost::numeric::ublas::matrix<double> FixedMotionWalker::transform(
    size_t timepos) {
  using namespace boost::numeric::ublas;
  matrix<double> T(4, 4);
  T = identity_matrix<double>(4);
