...
CoordinateSet::CoordinateSet() { m_size = 0; }

CoordinateSet::CoordinateSet(CoordinateSet& cs, IAtomselection* cs_selection,
                             IAtomselection* sub_selection) {
...
CartesianCoordinateSet::CartesianCoordinateSet(CartesianCoordinateSet& cs,
...
CartesianCoordinateSet::CartesianCoordinateSet(Frame& frame,
                                               IAtomselection* selection) {
  m_representation = CARTESIAN;
  ...
  if (m_size > frame.x.size()) {
    Err::Inst()->write(
        "Atom Index out of bounds for frame! Does the structure file match the "
        "frames?");
    throw;
  }
...
void CartesianCoordinateSet::transform(boost::numeric::ublas::matrix<double> T,
                                       IAtomselection* pcs_selection,
                                       IAtomselection* psub_selection) {
  IAtomselection& csel = *pcs_selection;
  IAtomselection& ssel = *psub_selection;
...
void CartesianCoordinateSet::rotate(CartesianCoor3D axis1,
                                    CartesianCoor3D axis2) {
  // the axis indicates the direction of the new x-axis!
  // we have to rotate the whole system, so that the old x-axis points towards
  // the new one
  ...
  for (size_t i = 0; i < m_size; ++i) {
    double c, s, r_00, r_01, r_02, r_10, r_11, r_12, r_20, r_21, r_22, xo, yo,
        zo;
