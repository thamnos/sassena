/** \file
This file contains a class which contains routines to write the in-memory-stored
trajectory to a file. This feature is mainly used for consistency check and
visualization of the artifical motions.
...
DCDCoordinateWriter::DCDCoordinateWriter(std::string file, size_t blocks,
                                         size_t entries)
    : blocks_(blocks), entries_(entries), file_(file) {}
...
void DCDCoordinateWriter::init() {
  ...
  marker = 0;
  out.write(p_marker, sizeof(int32_t));  // dummy 4
  int32_t one = 1;
  ...
  out.write((char*)(&marker_true),
            sizeof(int32_t));  // optional block 1, deactivate 14

  out.write((char*)(&marker_false),
            sizeof(int32_t));            // optional block 2, deactivate 15
...
void DCDCoordinateWriter::write(coor_t* data, size_t blockoffset,
                                size_t myblocks) {
  ...
  size_t blockbytesize = 2 * sizeof(int32_t) + 6 * sizeof(double) +
                         3 * 2 * sizeof(int32_t) + 3 * entries_ * sizeof(float);
  ...
  for (size_t i = 0; i < myblocks; ++i) {
    ...
    for (size_t k = 0; k < 3; ++k) {
      // prepare data
      for (size_t j = 0; j < entries_; ++j)
        p_entrydatabuffer[j] = data[i * entries_ * 3 + j * 3 + k];
      // write data
