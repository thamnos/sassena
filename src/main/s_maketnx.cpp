/** \file
This executable routine is used to generate a frame index file which may be
required for certain file formats. Sassena requires trajectory files to be
seekable (i.e. the frame offset position has to be known without reading the
trajectory first). Since some file format do not allow to precompute the offset
positions, they have to be indexed first.
...
void print_description() {
  ...
  Info::Inst()->write(
      "This binary generates a trajectory index to allow random access");
...
bool init_commandline(int argc, char** argv, po::variables_map& vm) {
  ...
  desc.add_options()("help", "produce help message")(
      ...
      "selection", po::value<string>(),
      "name of file containing a sub-selection of frames")(
      "index", po::value<string>(),
      "name of the trajectory index file (defaults to trajectory file name "
      "with txn extension)");
...
// second stage , allow command line parameters to overwrite defaults in the
// configuration file
//}
...
int main(int argc, char* argv[]) {
  ...
  if (!exists(trjpath)) {
    std::stringstream ss;
    ss << "No trajectory found with that path: " << trjpath.filename();
  ...
  if (vm.find("index") == vm.end()) {
    ...
    Warn::Inst()->write(
        "Index file name not specified. Defaulting to filename with tnx "
        "extension:");
  ...
  if (exists(ipath)) {
    ...
    while (true) {
      ssindex.str("");
      ssindex << ipath.filename() << ".backup-0"
    ...
    std::stringstream ss;
    ss << "Moving old index file to " << newidxpath.filename();
  ...
  if (vm.find("selection") != vm.end()) {
    ...
    Info::Inst()->write(string("Reducing the frame index to a number of ") +
                        boost::lexical_cast<string>(frame_selection.size()));
