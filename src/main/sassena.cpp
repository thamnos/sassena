...
\par Description:
Sassena is a software to compute X-ray and Neutron Scattering Intensities from
Molecular Dynamics Simulation Trajectories. It's purpose is to allow high
performance scattering calculations using massively parallel computers which
feature superior network infrastructures, e.g. infiniband.
...
void print_title() {
  Info::Inst()->write(
      "This software is being developed by Benjamin Lindner.                   "
      " ");
  Info::Inst()->write(
      "For help, suggestions or correspondense use:                            "
      " ");
  Info::Inst()->write(
      "ben@benlabs.net, Benjamin Lindner (Main Developer, Impl. & Maintenance) "
      " ");
  Info::Inst()->write(
      "franc@cmm.ki.si, Franci Merzel (Methodology)                            "
      " ");
  Info::Inst()->write(
      "For publications include the following references:                      "
      " ");
  Info::Inst()->write(
      "........................................................................"
      ".");
  Info::Inst()->write(
      "1. Sassena - X-ray and Neutron Scattering Calculated                    "
      " ");
  Info::Inst()->write(
      "  from Molecular Dynamics Trajectories using Massively Parallel "
      "Computers");
  Info::Inst()->write(
      "  published as a Computer Program in Physics Paper                      "
      " ");
  Info::Inst()->write(
      "........................................................................"
      ".");
  Info::Inst()->write(string("Version Information: ") +
                      string(Sassena_VERSIONSTRING));
  Info::Inst()->write("");
}
void print_description() {
  ...
  Info::Inst()->write(
      "This binary computes the scattering intensities directly from");
...
int main(int argc, char* argv[]) {
  ...
  // The rank 0 node is responsible for the progress output and to inform the
  // user
  // compute nodes should be silent all the time, except when errors occur.
  // In that case the size and the rank should be included into the error
  ...
  Info::Inst()->set_prefix(boost::lexical_cast<string>(world.rank()) +
                           string(".Info>>"));
  Warn::Inst()->set_prefix(boost::lexical_cast<string>(world.rank()) +
                           string(".Warn>>"));
  Err::Inst()->set_prefix(boost::lexical_cast<string>(world.rank()) +
                          string(".Err>>"));
  ...
  if (world.rank() == 0)
    Info::Inst()->write(
        "Exchanging sample, database & params information with compute "
        "nodes... ");
  ...
  boost::mpi::communicator scatter_comm =
      boost::mpi::communicator(world, world.group());
  ...
  if (world.rank() == 0) {
    ...
    p_hdf5writer = new HDF5WriterService(
        io_service, Params::Inst()->scattering.signal.filepath,
        sample.coordinate_sets.size());
  ...
  if (world.rank() == 0) {
    Info::Inst()->write(string("Checking ") +
                        Params::Inst()->scattering.signal.filepath +
                        string(" for old results."));
    ...
    for (size_t i = 0; i < Params::Inst()->scattering.qvectors.size(); ++i) {
      if (oldqvectors_set.find(Params::Inst()->scattering.qvectors[i]) ==
          oldqvectors_set.end()) {
    ...
    if (qvectors.size() != Params::Inst()->scattering.qvectors.size()) {
      Info::Inst()->write(
          string("Found ") +
          boost::lexical_cast<string>(
              Params::Inst()->scattering.qvectors.size() - qvectors.size()) +
          string(" old qvectors."));
      Info::Inst()->write(
          string("Total number of qvectors to compute reduced from ") +
          boost::lexical_cast<string>(
              Params::Inst()->scattering.qvectors.size()) +
          string(" to ") + boost::lexical_cast<string>(qvectors.size()) +
          string("."));
    }
  ...
  if (scatter_comm.rank() == 0) {
    ...
    fileservice_port_str =
        boost::lexical_cast<string>(p_hdf5writer->get_endpoint().port());
    monitorservice_port_str =
        boost::lexical_cast<string>(p_monitorservice->get_endpoint().port());
  }
  if (world.rank() == 0)
    Info::Inst()->write("Broadcasting service information");

  mpi::wrapper::broadcast_class<std::string>(scatter_comm, host_str, 0);
  mpi::wrapper::broadcast_class<std::string>(scatter_comm, fileservice_port_str,
                                             0);
  mpi::wrapper::broadcast_class<std::string>(scatter_comm,
                                             monitorservice_port_str, 0);

  if (world.rank() == 0) {
    Info::Inst()->write(string("Server host name    : ") + host_str);
    Info::Inst()->write(string("FileService Port    : ") +
                        fileservice_port_str);
    Info::Inst()->write(string("MonitorService Port : ") +
                        monitorservice_port_str);
  }
  ...
  boost::asio::ip::tcp::resolver::query query(
      boost::asio::ip::tcp::v4(), host_str, fileservice_port_str,
      boost::asio::ip::resolver_query_base::numeric_service);
  boost::asio::ip::tcp::resolver::iterator it = resolver.resolve(query);
  boost::asio::ip::tcp::endpoint fileservice_endpoint(*it);
  // monitor
  boost::asio::ip::tcp::resolver::query query2(
      boost::asio::ip::tcp::v4(), host_str, monitorservice_port_str,
      boost::asio::ip::resolver_query_base::numeric_service);
  ...
  p_ScatterDevice =
      ScatterDeviceFactory::create(scatter_comm, sample, fileservice_endpoint,
                                   monitorservice_endpoint, qvectors);
  ...
  if (world.rank() == 0) {
    Info::Inst()->write(
        string("Aggregating timing information for performance analysis..."));
  }
  ...
  PerformanceAnalyzer perfanal(
      world, performance_timer);  // collect timing information from everybody.
  if (p_ScatterDevice != NULL) delete p_ScatterDevice;

  if (world.rank() == 0) {
    perfanal.report();
    perfanal.report_relative(timer.sum("total") * world.size());
    Info::Inst()->write(string("Total runtime (s): ") +
                        boost::lexical_cast<string>(timer.sum("total")));
