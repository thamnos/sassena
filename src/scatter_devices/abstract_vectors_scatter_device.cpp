...
void AbstractVectorsScatterDevice::print_pre_runner_info() {
  if (Params::Inst()->debug.print.orientations) {
    if (Params::Inst()->scattering.average.orientation.vectors.size() > 0) {
      if (allcomm_.rank() == 0) {
        ...
        for (size_t i = 0;
             i < Params::Inst()->scattering.average.orientation.vectors.size();
             i++) {
          string qvector = "";
          qvector += boost::lexical_cast<string>(
              Params::Inst()->scattering.average.orientation.vectors[i].x);
          qvector += " ";
          qvector += boost::lexical_cast<string>(
              Params::Inst()->scattering.average.orientation.vectors[i].y);
          qvector += " ";
          qvector += boost::lexical_cast<string>(
              Params::Inst()->scattering.average.orientation.vectors[i].z);
          cout << qvector << endl;
        }
      }
    }
  ...
  if (allcomm_.rank() == 0) {
    ...
    if (Params::Inst()->scattering.average.orientation.vectors.size() > 0) {
      Info::Inst()->write(
          string("Orientational averaging includes ") +
          boost::lexical_cast<string>(
              Params::Inst()->scattering.average.orientation.vectors.size()) +
          string(" vectors"));
    }
  }
}
...
double AbstractVectorsScatterDevice::progress() {
  ...
  // scale2 is not preserved for cases where the size is dependent on the vector
  // (e.g. 0 does not require any "orientations")
  ...
  if (Params::Inst()->scattering.average.orientation.vectors.size() > 0) {
    if (Params::Inst()->scattering.average.orientation.vectors.type == "file") {
      ...
      for (size_t i = 0;
           i < Params::Inst()->scattering.average.orientation.vectors.size();
           ++i) {
        subvector_index_.push_back(
            ql * Params::Inst()->scattering.average.orientation.vectors[i]);
      }
    } else if (Params::Inst()->scattering.average.orientation.vectors.type ==
               "sphere") {
      double ql = q.length();
      ...
      if (qcylinder.r == 0) {
        ...
        for (size_t i = 0;
             ...
             ++i) {
          CartesianCoor3D& vec =
              Params::Inst()->scattering.average.orientation.vectors[i];
          // it's "easier" to just construct the vectors from the base, then to
          // do coordinate transformations
          CartesianCoor3D qnew =
              qcylinder.z * base[2] +
              qcylinder.r * (vec.x * base[0] + vec.y * base[1]);
      ...
      if (Params::Inst()->debug.print.orientations) {
        if (subvector_index_.size() > 0) {
          if (allcomm_.rank() == 0) {
            Info::Inst()->write(
                "Final Qvectors orientations used for averaging: ");
