...
class SampleStructureParameters {
 ...
 public:
  ...
  std::string write_xml(int pad = 0) {
    ...
    ss << std::string(pad, ' ') << "<format>" << format << "</format>"
       << std::endl;
    return ss.str();
  }
};
...
/**
Section which defines a generic selection (used as parent by specific
selections)
*/
...
class SampleIndexSelectionParameters : public SampleSelectionParameters {
  ...
  template <class Archive>
  void serialize(Archive& ar, const unsigned int version) {
    ar& boost::serialization::base_object<SampleSelectionParameters,
 ...
 public:
  ...
  SampleIndexSelectionParameters() {}
  SampleIndexSelectionParameters(std::vector<size_t> ids)
      : SampleSelectionParameters("index"), ids_(ids) {}
  std::string write_xml(int pad = 0) {
    std::stringstream ss;
    ss << SampleSelectionParameters::write_xml(pad);
    for (size_t i = 0; i < ids_.size(); ++i) {
      ss << std::string(pad, ' ') << "<index>" << ids_[i] << "</index>"
         << std::endl;
    }
    return ss.str();
  }
...
class SampleRangeSelectionParameters : public SampleSelectionParameters {
 ...
 public:
  ...
  SampleRangeSelectionParameters() {}
  SampleRangeSelectionParameters(size_t from, size_t to)
      : SampleSelectionParameters("range"), from_(from), to_(to) {}
  std::string write_xml(int pad = 0) {
    std::stringstream ss;
    ss << SampleSelectionParameters::write_xml(pad);
...
/**
Section which defines a selection based on a lexical pattern (regular expression
matching atom labels)
*/
class SampleLexicalSelectionParameters : public SampleSelectionParameters {
 ...
 public:
  ...
  SampleLexicalSelectionParameters() {}
  SampleLexicalSelectionParameters(std::string expression)
      : SampleSelectionParameters("lexical"), expression_(expression) {}
  std::string write_xml(int pad = 0) {
    std::stringstream ss;
    ss << SampleSelectionParameters::write_xml(pad);
    ss << std::string(pad, ' ') << "<expression>" << expression_
       << "</expression>" << std::endl;
    return ss.str();
  }
};
...
class SampleFileSelectionParameters : public SampleSelectionParameters {
  ...
  void serialize(Archive& ar, const unsigned int version) {
    ar& boost::serialization::base_object<SampleSelectionParameters,
                                          SampleFileSelectionParameters>(*this);
    ar& file_;
    ar& filepath_;
    ar& format_;
    ar& selector_;
 ...
 public:
  ...
  SampleFileSelectionParameters(std::string file, std::string format,
                                std::string selector, std::string expression)
      : SampleSelectionParameters("file"),
        file_(file),
        format_(format),
        selector_(selector),
  ...
  std::string write_xml(int pad = 0) {
    ...
    ss << std::string(pad, ' ') << "<format>" << format_ << "</format>"
       << std::endl;
    ss << std::string(pad, ' ') << "<selector>" << selector_ << "</selector>"
       << std::endl;
    ss << std::string(pad, ' ') << "<expression>" << expression_
       << "</expression>" << std::endl;
    return ss.str();
 ...
 public:
  ...
  std::string write_xml(int pad = 0) {
    ...
    ss << std::string(pad, ' ') << "<format>" << format << "</format>"
       << std::endl;
    ss << std::string(pad, ' ') << "<file>" << file << "</file>" << std::endl;
    // don't write location specific filepath or indexpath! not well defined
    // behavior
    if (!index_default) {
      ss << std::string(pad, ' ') << "<index>" << index << "</index>"
         << std::endl;
    }
    ss << std::string(pad, ' ') << "<first>" << first << "</first>"
       << std::endl;
    if (last_set) {
      ss << std::string(pad, ' ') << "<last>" << last << "</last>" << std::endl;
    }
    ss << std::string(pad, ' ') << "<clones>" << clones << "</clones>"
       << std::endl;
    ss << std::string(pad, ' ') << "<stride>" << stride << "</stride>"
       << std::endl;
    return ss.str();
  }
};
...
class SampleFramesetsParameters : public std::vector<SampleFramesetParameters> {
 private:
  ...
  template <class Archive>
  void serialize(Archive& ar, const unsigned int version) {
    ar& boost::serialization::base_object<std::vector<SampleFramesetParameters>,
                                          SampleFramesetsParameters>(*this);
  }
  ///////////////////
...
/**
Section which stores reference information which may be required during some
...
class SampleMotionReferenceParameters {
 ...
 public:
  ...
  std::string write_xml(int pad = 0) {
    ...
    ss << std::string(pad, ' ') << "<format>" << format << "</format>"
       << std::endl;
    ss << std::string(pad, ' ') << "<frame>" << frame << "</frame>"
       << std::endl;
    return ss.str();
  }
};
...
class SampleMotionParameters {
 ...
 public:
  ...
  std::string write_xml(int pad = 0) {
    std::stringstream ss;
    ss << std::string(pad, ' ') << "<type>" << type << "</type>" << std::endl;
    ss << std::string(pad, ' ') << "<displace>" << displace << "</displace>"
       << std::endl;
    ss << std::string(pad, ' ') << "<frequency>" << frequency << "</frequency>"
       << std::endl;
    ss << std::string(pad, ' ') << "<radius>" << radius << "</radius>"
       << std::endl;
    ss << std::string(pad, ' ') << "<selection>" << selection << "</selection>"
       << std::endl;
    ss << std::string(pad, ' ') << "<seed>" << seed << "</seed>" << std::endl;
    ss << std::string(pad, ' ') << "<sampling>" << sampling << "</sampling>"
       << std::endl;
...
/**
Section which stores alignment information, which is applied during the staging
...
class SampleAlignmentParameters {
 ...
 public:
  ...
  std::string write_xml(int pad = 0) {
    ...
    ss << std::string(pad, ' ') << "<selection>" << selection << "</selection>"
       << std::endl;
    ss << std::string(pad, ' ') << "<order>" << order << "</order>"
       << std::endl;
    ss << std::string(pad, ' ') << "<referemce>" << std::endl;
    ss << reference.write_xml(pad + 1);
    ss << std::string(pad, ' ') << "</referemce>" << std::endl;
 ...
 public:
  ...
  std::string write_xml(int pad = 0) {
    ...
    for (std::map<std::string, SampleSelectionParameters*>::iterator i =
      ...
      ss << std::string(pad + 2, ' ') << "<name>" << i->first << "</name>"
         << std::endl;
...
*/
class ScatteringAverageOrientationVectorsParameters
    : public std::vector<CartesianCoor3D> {
 private:
  /////////////////// MPI related
  // make this class serializable to
  // allow sample to be transmitted via MPI
 ...
 public:
  ...
  std::string write_xml(int pad = 0) {
    std::stringstream ss;
    ss << std::string(pad, ' ') << "<type>" << type << "</type>" << std::endl;
    ss << std::string(pad, ' ') << "<algorithm>" << algorithm << "</algorithm>"
       << std::endl;
    ss << std::string(pad, ' ') << "<resolution>" << resolution
       << "</resolution>" << std::endl;
    ss << std::string(pad, ' ') << "<file>" << file << "</file>" << std::endl;
    ss << std::string(pad, ' ') << "<seed>" << seed << "</seed>" << std::endl;
    return ss.str();
...
/**
Section which is used to store the used multipole identifiers when multipole
based orientational averaging is performed
*/
class ScatteringAverageOrientationMultipoleMomentsParameters
    : public std::vector<std::pair<long, long> > {
 private:
  /////////////////// MPI related
  // make this class serializable to
  // allow sample to be transmitted via MPI
...
/**
Section which defines further processing of the scattering signal, e.g.
autocorrelation
*/
class ScatteringDspParameters {
 ...
 public:
  ...
  std::string write_xml(int pad = 0) {
    std::stringstream ss;
    ss << std::string(pad, ' ') << "<type>" << type << "</type>" << std::endl;
    ss << std::string(pad, ' ') << "<method>" << method << "</method>"
       << std::endl;
    return ss.str();
  }
};
...
class ScatteringVectorsScanParameters {
 ...
 public:
  ScatteringVectorsScanParameters()
      : points(100), exponent(1.0), basevector(CartesianCoor3D(1, 0, 0)) {}
  ...
  std::string write_xml(int pad = 0) {
    ...
    ss << std::string(pad, ' ') << "<points>" << points << "</points>"
       << std::endl;
    ss << std::string(pad, ' ') << "<exponent>" << exponent << "</exponent>"
       << std::endl;
...
class StagerParameters {
 ...
 public:
  ...
  std::string write_xml(int pad = 0) {
    ...
    ss << std::string(pad, ' ') << "<format>" << format << "</format>"
       << std::endl;
    ss << std::string(pad, ' ') << "<target>" << target << "</target>"
       << std::endl;
...
/**
Section which stores parameters affecting the memory limitations of the file
writer service
*/
class LimitsServicesSignalMemoryParameters {
 ...
 public:
  ...
  std::string write_xml(int pad = 0) {
    std::stringstream ss;
    ss << std::string(pad, ' ') << "<server>" << server << "</server>"
       << std::endl;
    ss << std::string(pad, ' ') << "<client>" << client << "</client>"
       << std::endl;
    return ss.str();
  }
};
...
class LimitsServicesSignalTimesParameters {
 ...
 public:
  ...
  std::string write_xml(int pad = 0) {
    std::stringstream ss;
    ss << std::string(pad, ' ') << "<serverflush>" << serverflush
       << "</serverflush>" << std::endl;
    ss << std::string(pad, ' ') << "<clientflush>" << clientflush
       << "</clientflush>" << std::endl;
    return ss.str();
  }
};

/**
Section which stores parameters affecting the file writer service, which writes
results to the signal file
*/
...
/**
Section which stores parameters affecting the monitoring service, which reports
...
class LimitsServicesMonitorParameters {
 ...
 public:
  ...
  std::string write_xml(int pad = 0) {
    std::stringstream ss;
    ss << std::string(pad, ' ') << "<delay>" << delay << "</delay>"
       << std::endl;
    ss << std::string(pad, ' ') << "<sampling>" << sampling << "</sampling>"
       << std::endl;
    return ss.str();
...
class LimitsComputationMemoryParameters {
 ...
 public:
  ...
  std::string write_xml(int pad = 0) {
    std::stringstream ss;
    ss << std::string(pad, ' ') << "<result_buffer>" << result_buffer
       << "</result_buffer>" << std::endl;
    ss << std::string(pad, ' ') << "<alignpad_buffer>" << alignpad_buffer
       << "</alignpad_buffer>" << std::endl;
    ss << std::string(pad, ' ') << "<exchange_buffer>" << exchange_buffer
       << "</exchange_buffer>" << std::endl;
    ss << std::string(pad, ' ') << "<signal_buffer>" << signal_buffer
       << "</signal_buffer>" << std::endl;
    ss << std::string(pad, ' ') << "<scale>" << scale << "</scale>"
       << std::endl;
    return ss.str();
  }
};
...
class LimitsComputationParameters {
 ...
 public:
  ...
  std::string write_xml(int pad = 0) {
    std::stringstream ss;
    ss << std::string(pad, ' ') << "<threads>" << threads << "</threads>"
       << std::endl;
    ss << std::string(pad, ' ') << "<processes>" << processes << "</processes>"
       << std::endl;
    ss << std::string(pad, ' ') << "<cores>" << cores << "</cores>"
       << std::endl;
    ss << std::string(pad, ' ') << "<memory>" << std::endl;
    ss << memory.write_xml(pad + 1);
    ss << std::string(pad, ' ') << "</memory>" << std::endl;
...
class LimitsDecompositionPartitionsParameters {
 ...
 public:
  ...
  std::string write_xml(int pad = 0) {
    std::stringstream ss;
    ss << std::string(pad, ' ') << "<automatic>" << automatic << "</automatic>"
       << std::endl;
...
/**
Section which stores parameters used for partitioning the computation among the
...
class LimitsDecompositionParameters {
 ...
 public:
  ...
  std::string write_xml(int pad = 0) {
    std::stringstream ss;
    ss << std::string(pad, ' ') << "<utilization>" << utilization
...
/**
Section which stores parameters used during the writing of the signal output
file
*/
class LimitsSignalParameters {
 ...
 public:
  ...
  std::string write_xml(int pad = 0) {
    std::stringstream ss;
    ss << std::string(pad, ' ') << "<chunksize>" << chunksize << "</chunksize>"
       << std::endl;
    return ss.str();
  }
};
...
class LimitsStageMemoryParameters {
 ...
 public:
  ...
  std::string write_xml(int pad = 0) {
    ...
    ss << std::string(pad, ' ') << "<buffer>" << buffer << "</buffer>"
       << std::endl;
    return ss.str();
  }
};
...
class DebugMonitorParameters {
 ...
 public:
  ...
  std::string write_xml(int pad = 0) {
    std::stringstream ss;
    ss << std::string(pad, ' ') << "<update>" << update << "</update>"
       << std::endl;
    return ss.str();
  }
};
...
class DebugPrintParameters {
 ...
 public:
  ...
  std::string write_xml(int pad = 0) {
    std::stringstream ss;
    ss << std::string(pad, ' ') << "<orientations>" << orientations
...
/**
Section which stores IO write specific debug parameters. Used to tune the
frequency by which the results are written to the signal file. Can also be used
to avoid writing to the signal file.
*/
class DebugIowriteParameters {
 ...
 public:
  ...
  std::string write_xml(int pad = 0) {
    std::stringstream ss;
    ss << std::string(pad, ' ') << "<write>" << write << "</write>"
       << std::endl;
    ss << std::string(pad, ' ') << "<buffer>" << buffer << "</buffer>"
       << std::endl;
    return ss.str();
...
class DebugParameters {
 ...
 public:
  ...
  std::string write_xml(int pad = 0) {
    std::stringstream ss;
    ss << std::string(pad, ' ') << "<timer>" << timer << "</timer>"
       << std::endl;
    ss << std::string(pad, ' ') << "<barriers>" << barriers << "</barriers>"
       << std::endl;
...
/**
This is a wrapper class to interface the settings implementation. The rational
is to
move all possible configuration errors towards the initialization of the
software
preferably the 'parameters' class checks for all required settings and
implementents
default values
...
class Params {
 private:
  ...
  std::vector<char>
      config;  // contains complete text copy of the input configuration
 ...
 public:
  ...
  ~Params(){};  // it is said some compilers have problems w/ private
                // destructors.
