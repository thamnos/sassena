...
class Sample {
 ...
 public:
  ...
  Sample(std::string filename, std::string fileformat)
      : atoms(filename, fileformat) {}

  void init();

  // default routine for reading structure information from file.
  void add_atoms(std::string filename, std::string fileformat) {
    return atoms.add(filename, fileformat);
  }
};
